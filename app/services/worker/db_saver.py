"""
    이 모듈은 treatment_mapper에서 치료 ID가 매핑된 이후,
    파싱된 치료 데이터를 데이터베이스에 저장하는 역할을 합니다.
"""
    ...
    try:
        ...
        for item in mapped_treatments:
            ...
            db.add(OrderTreatment(
                ...
                assigned_doctor_id=assignment.get("assigned_doctor_id"),
                assigned_at=db.query(db.func.now()).scalar() if assignment.get("assigned_doctor_id") else None
            ))
